...
CREATE TABLE `area` (
  ...
  `unidade_de_medida` SET('metros','hectares') NOT NULL COMMENT 'Informa se o tamanho está cadastrado em metros quadrados ou hectares.',
  PRIMARY KEY (`id`),
...
CREATE TABLE `sensor` (
  ...
  `unidade_de_medida` SET('percentual','mg/kg','ppm') NOT NULL COMMENT 'Unidade de medida usada pelo sensor, cada tipo de sensor tem uma unidade padrão.',
  `tipo` SET('sensor de umidade','sensor de ph','sensor de potássio','sensor de fósforo', 'escala') NOT NULL COMMENT 'Tipo do sensor: pH, umidade, potássio, fósforo.',
...
CREATE TABLE `manejo` (
  ...
  `tipo` SET('irrigação', 'aplicação de fertilizantes', 'correção de solo') NOT NULL COMMENT 'Tipo do produto: irrigação, aplicação de fertilizantes, correção de solo.',
  PRIMARY KEY (`id`)
